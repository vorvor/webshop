...
class AddTermsToVocabularyForm extends FormBase {
  ...
  /**
   * The Taxonomy Manager Settings.
   *
   * @var \Drupal\Core\Config\Config
   */
  protected $taxonomyManagerSetting;
   ...
   */
  public function __construct(TaxonomyManagerHelper $taxonomy_manager_helper, ConfigFactoryInterface $configFactory) {
    $this->taxonomyManagerHelper = $taxonomy_manager_helper;
    $this->configFactory = $configFactory;
    $this->taxonomyManagerSetting = $configFactory->get('taxonomy_manager.settings');
  }
  ...
  public function buildForm(array $form, FormStateInterface $form_state, ?VocabularyInterface $taxonomy_vocabulary = NULL, $parents = []) {
    ...
    $description_delimiter = $this->taxonomyManagerSetting->get('taxonomy_manager_description_delimiter');
    if (!$description_delimiter) {
    ...
    $form['mass_add'] = [
      ...
      '#description' => $this->t("One term per line. Child terms can be prefixed with a
        ...
        You can also use  to split term name and term description.
        Example:<br />
        animals<br />
        -canine<br />
        --dog<br />
        --wolf@delimiterDescribe your wolf<br />
        -feline<br />
        --cat@delimiterDescribe your cat", ['@delimiter' => $description_delimiter]),
      '#rows' => 10,
      '#required' => TRUE,
    ];
    $form['keep_order'] = [
